...
void bfs(int s) {
    ...
    while(!q.empty()) {
        ...
        处理操作e                         //判断是否满足条件
        队首元素出队pop()
        将e的下一层未曾入队结点加入队列q    //需要在外部准备一个判断结点是否访问过的数据结构（数组或者哈希表）
    }
}
```

    A Jugs：
    B DFS or BFS?：躲避下落的石头。
        本题需要在BFS的基础上做一点改进。
    C 【宽搜入门】8数码难题：寻找最快的解法（明显是使用BFS）。
        ！使用bfs的要点
    D 【宽搜入门】魔板：简单BFS问题
        ！常常需要使用pair<strin, string>，一个表示“局面”，一个表示变化路径，同时为了剪枝还需要准备一个set<string>来检查是否访问过
    E 【宽搜入门】巧妙取量
        ！！！一旦隔了一段时间，就忘记提交前删除测试函数了
        ！！！BFS时源节点在创建的时候也要在记录访问情况的hashTable中设置为true
...
6-1-A：答案错误81
8-1-D：懒得做
8-2-A：思路很长的技巧题，针对性不太强
8-2-C：代码没有构思好就动笔了，太浮躁，应该先好好设计一番
```